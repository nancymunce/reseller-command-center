...
function itemToDatabase(item) {
  return {
    ...
    listing_photo_paths: item.listingPhotoPaths || []
  };
}
...
function databaseToItem(row) {
  return {
    ...
    ebayCategoryId: row.ebay_category_id || "", ebayConditionId: row.ebay_condition_id || "", ebayItemSpecifics: row.ebay_item_specifics || {}, listingPhotoPaths: row.listing_photo_paths || []
  };
}
...
function ebayDraftIssues(i){
 ...
 if(!(i.listingPhotoPaths||[]).length) issues.push("photo");
 return issues;
}
function draftWorkState(i){
 const issues=ebayDraftIssues(i);
 if(i.draftStatus==="exported")return "exported";
 if(i.draftStatus==="approved")return "approved";
 if(!issues.length)return "ready";
 return "needs";
}
function draftPhotoCount(i){return (i.listingPhotoPaths||[]).length;}
...
function renderMasterDrafts() {
  ...
  const allDrafts=items.filter(i=>["draft","approved","exported"].includes(i.draftStatus));
...
function suggestEbayConditionFromDraft(){
 ...
 if(!id){toast("Condition text is not specific enough to suggest an eBay condition.");return;}
 $("masterDraftEbayConditionId").value=id;
...
function nextReviewDraftId(currentId){
 ...
 const pos=queue.findIndex(i=>i.id===currentId);
 if(pos<0)return queue[0]?.id||"";
 return queue[(pos+1)%queue.length]?.id||"";
}
...
async function saveMasterDraftAndNext(){
 const current=$("masterDraftId").value,next=nextReviewDraftId(current);
 const saved=await saveMasterDraft(false);if(!saved)return;
 if(next&&next!==current)openMasterDraft(next);else toast("Draft saved — no other review drafts in the queue.");
}
...
$("approveNextMasterDraftBtn")?.addEventListener("click",approveMasterDraftAndNext);
