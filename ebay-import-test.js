...
async function runEbayAnalysis(){try{$("readyText").textContent="Analyzing reports…";records=[];const warnings=[];for(const [inputId,,expected] of slots){const file=$(inputId).files[0];if(!file)throw new Error("Please select all three reports.");const rows=csvRows(await file.text()),d=detect(rows);if(!d){warnings.push(file.name+": report type not recognized.");continue}if(d.type!==expected)warnings.push(file.name+": expected "+expected+" report but detected "+d.type+".");objects(rows,d.header).forEach((o,i)=>{const n=normalize(d.type,o,i);if(d.type==="sold"&&!n.id&&!n.title&&n.price===0)return;records.push(n)})}render(3,warnings);$("readyText").textContent="Analysis complete — review the results below.";$("resultsPanel").scrollIntoView({behavior:"smooth",block:"start"});}catch(e){$("readyText").textContent="ANALYSIS ERROR: "+e.message;}} window.runEbayAnalysis=runEbayAnalysis;
